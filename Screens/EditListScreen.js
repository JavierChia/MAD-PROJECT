...
export default function App({ route, navigation }) {
  ...
  useEffect(() => {
    const readData = async () => {
      try {
        var list = await getDoc(
          doc(db, `users/${uid}/Lists/${route.params.listID}`)
        );
        var tasks = await getDocs(
          collection(db, `users/${uid}/Lists/${route.params.listID}/Tasks`)
        );
  ...
  const handleCreate = async () => {
    try {
      if (listName == "") {
        ...
        //add tasks from other lists into TASKS
